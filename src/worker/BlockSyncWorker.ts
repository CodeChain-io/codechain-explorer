...
export class BlockSyncWorker {
    ...
    private async sync() {
        ...
        if (latestSyncBlockNumber === -1) {
            ...
            console.log(
                ...
                latestSyncBlockNumber
            );
        }
        console.log(
            "lastest codechain block number : %d",
            latestCodechainBlockNumber
        );
        while (latestSyncBlockNumber < latestCodechainBlockNumber) {
            const nextBlockIndex: number = latestSyncBlockNumber + 1;
            const nextBlock: Block = await this.codeChainAgent.getBlock(
                nextBlockIndex
            );
            if (latestSyncBlockNumber > 0) {
                const lastSyncBlock: BlockDoc = await this.elasticSearchAgent.getBlock(
                    latestSyncBlockNumber
                );
                if (nextBlock.parentHash.value !== lastSyncBlock.hash) {
                    latestSyncBlockNumber = await this.checkRetractAndReturnSyncNumber(
                        latestSyncBlockNumber
                    );
                    continue;
                }
    ...
    private indexingPendingParcel = async () => {
        ...
        const pendingParcelHashList = _.map(
            pendingParcels,
            p => p.hash().value
        );
        const removedPendingParcels = _.filter(
            indexedParcels,
            indexedParcel =>
                !_.includes(pendingParcelHashList, indexedParcel.parcel.hash)
        );
        await Promise.all(
            _.map(removedPendingParcels, async removedPendingParcel => {
                ...
                if (blockedParcel) {
                    return this.elasticSearchAgent.removePendingParcel(
                        new H256(removedPendingParcel.parcel.hash)
                    );
                } else {
                    return this.elasticSearchAgent.deadPendingParcel(
                        new H256(removedPendingParcel.parcel.hash)
                    );
                }
            })
        ...
        const indexedPendingParcelHashList = _.map(
            indexedParcels,
            p => p.parcel.hash
        );
        ...
        await Promise.all(
            _.map(newPendingParcels, async pendingParcel => {
                const pendingParcelDoc = await this.typeConverter.fromPendingParcel(
                    pendingParcel
                );
                return this.elasticSearchAgent.indexPendingParcel(
                    pendingParcelDoc
                );
            })
        );
        ...
        const deadPendingParcelHashList = _.map(
            deadPendingParcels,
            p => p.parcel.hash
        );
        ...
        await Promise.all(
            _.map(revivalPendingParcels, async revivalPendingParcel => {
                return this.elasticSearchAgent.revialPendingParcel(
                    revivalPendingParcel.hash()
                );
            })
        );
    };
    ...
    private checkRetractAndReturnSyncNumber = async (
        ...
        while (currentBlockNumber > -1) {
            const lastSynchronizedBlock: BlockDoc = await this.elasticSearchAgent.getBlock(
                currentBlockNumber
            );
            const codechainBlock: Block = await this.codeChainAgent.getBlock(
                currentBlockNumber
            );
    ...
    private indexingNewBlock = async (nextBlock: Block) => {
        ...
        if (blockDoc.number === 0) {
            const addressListJob = _.map(
                this.config.genesisAddressList[
                    process.env.CODECHAIN_CHAIN || "solo"
                ],
                async address => {
                    const balance = await this.codeChainAgent.getBalance(
                        address,
                        0
                    );
                    return {
            ...
            const updateAddressJob = _.map(addressList, address =>
                this.elasticSearchAgent.increaseBalance(
                    address.address,
                    address.balance
                )
            );
        ...
        await Promise.all(
            _.map(
                transactions,
                async transaction =>
                    await this.elasticSearchAgent.indexTransaction(transaction)
            )
        );
        // index log
        ...
        if (parcelCount > 0) {
            ...
            const paymentParcelCount = _.filter(blockDoc.parcels, p =>
                Type.isPaymentDoc(p.action)
            ).length;
            await this.elasticSearchAgent.increaseLogCount(
                dateString,
            ...
            const serRegularKeyParcelCount = _.filter(blockDoc.parcels, p =>
            ...
            const changeShardStateParcelCount = _.filter(blockDoc.parcels, p =>
        ...
        if (txCount > 0) {
            ...
            const assetMintTxCount = _.filter(transactions, tx =>
            ...
            const assetTransferTxCount = _.filter(transactions, tx =>
        ...
        // index account
        await this.elasticSearchAgent.increaseBalance(
            blockDoc.author,
            blockDoc.miningReward
        );

        // Resolve sequentially
        for (const parcel of blockDoc.parcels) {
            await this.elasticSearchAgent.decreaseBalance(
                parcel.sender,
                parcel.fee
            );
        }

        const paymentParcels = _.filter(blockDoc.parcels, parcel =>
            Type.isPaymentDoc(parcel.action)
        );
        const succeedPaymentParcelJob = _.map(paymentParcels, async parcel => {
            const invoices = (await this.codeChainAgent.getParcelInvoice(
                new H256(parcel.hash)
            )) as Invoice;
            if (invoices.success) {
                return parcel;
            } else {
        ...
        const succeedPaymentParcels = _.compact(
            await Promise.all(succeedPaymentParcelJob)
        );
        // Resolve sequentially
        for (const parcel of succeedPaymentParcels) {
            const paymentAction = parcel.action as PaymentDoc;
            await this.elasticSearchAgent.increaseBalance(
                paymentAction.receiver,
    ...
    private retractBlock = async (lastSynchronizedBlock: BlockDoc) => {
        await this.elasticSearchAgent.retractBlock(
            new H256(lastSynchronizedBlock.hash)
        );
        await Promise.all(
            _.map(
                lastSynchronizedBlock.parcels,
                async parcel =>
                    await this.elasticSearchAgent.retractParcel(
                        new H256(parcel.hash)
                    )
            )
        );
        ...
        await Promise.all(
            _.map(
                transactions,
                async transaction =>
                    await this.elasticSearchAgent.retractTransaction(
        ...
        if (parcelCount > 0) {
            ...
            const setRegularKeyParcelCount = _.filter(
                lastSynchronizedBlock.parcels,
                p => Type.isSetRegularKeyDoc(p.action)
            ).length;
            await this.elasticSearchAgent.decreaseLogCount(
                dateString,
                LogType.PARCEL_SET_REGULAR_KEY_COUNT,
            ...
            const changeShardStateParcelCount = _.filter(
                lastSynchronizedBlock.parcels,
                p => Type.isChangeShardStateDoc(p.action)
            ).length;
            await this.elasticSearchAgent.decreaseLogCount(
                dateString,
                LogType.PARCEL_CHANGE_SHARD_STATE_COUNT,
        ...
        if (txCount > 0) {
            ...
            const assetMintTxCount = _.filter(txs, tx =>
                Type.isAssetMintTransactionDoc(tx)
            ).length;
            await this.elasticSearchAgent.decreaseLogCount(
                dateString,
            ...
            const assetTransferTxCount = _.filter(txs, tx =>
                Type.isAssetTransferTransactionDoc(tx)
            ).length;
            await this.elasticSearchAgent.decreaseLogCount(
                dateString,
        ...
        await this.elasticSearchAgent.decreaseBalance(
            blockMiner,
            lastSynchronizedBlock.miningReward
        );
        ...
        const paymentParcels = _.filter(lastSynchronizedBlock.parcels, parcel =>
            Type.isPaymentDoc(parcel.action)
        );
        const succeedPaymentParcelJob = _.map(paymentParcels, async parcel => {
            const invoices = await this.codeChainAgent.getParcelInvoice(
